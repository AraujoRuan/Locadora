    ...
    public class Phone : ValueObject
    {
        ...
        public Phone(string number)
        {
            ...
            if (string.IsNullOrEmpty(number))
                throw new Exception("O numero não pode ser vazio");

            if (Number.Length < 5)
                throw new Exception("O numero deve conter mais que 5 digito");

            if (!NumberRegex().IsMatch(Number))
                throw new Exception("O numero inserido não está com formato válido");
